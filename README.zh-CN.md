...
默认用 `lite` 处理沿用现有架构的边界清楚改动，包括局部行为变化。只有明确命中完整新能力、
共享核心重构、破坏性演进、广泛回归或协调式发布等工程触发器，才进入
`demo → prd → testcase → build`。风险和发布只增加安全检查，本身不选择 Build。

它的核心承诺是：**边界清楚的工作用聚焦证据快速交付；结构性复杂的 Build 工作先让人批准
黑盒用例，再用 runner 自动产出的版本绑定证据证明。**
...
默认先走 `lite`，再检查是否命中明确的 Build 触发器。不得根据预计耗时、笼统的高风险、
行为变化或发布意图选择 Build。

| 路径 | 什么时候用 | 会做什么 | 不会做什么 |
| --- | --- | --- | --- |
| `vibe` | 你明确只要最快代码，并亲自验收 | 直接修改，交给人看 diff | 不测试、不运行、不生成证据，不能声称可发布 |
| `lite` | 沿用现有架构的边界清楚改动，包括局部行为变化 | 聚焦检查、对应风险保护和按比例真实入口证据 | 不会仅因行为变化或需要发布就制造 Feature 工件 |
| `build` | 完整新能力、共享核心重构、破坏性演进、广泛回归或协调式发布 | 已批准 Core-Case、可运行切片、评审、新鲜收据 | 未指出具体触发器时不能选择 Build |
| 按旅程拆分 | 包含多个可独立使用的结果 | 为清晰度和独立验收拆开旅程 | 不以预计工时作为停止门禁 |

![OPC-Develop 工程深度选路图](assets/opc-develop-routing.zh-CN.png)

这张图只是**选路图和单次增量流程**，不是全景架构图。

### 4. Build 必经产品定义，架构仍按需
...
公共/单向边界变化时插入。这条链只适用于确实命中 Build 触发器的工作；意图清楚的局部行为
变化仍走 `lite`，产品意图不清楚时用 `brainstorm`/`demo` 澄清，而不是自动升级 Build。
...
| Skill | 典型场景 | 主要结果 | 下一步 |
| --- | --- | --- | --- |
| `vibe` | 一次性实验、你明确接受未验证代码 | 代码 diff + “未运行测试”说明 | 人工检查；发布前改走已验证的 Lite 或 Build |
| `lite` | 局部 bug、行为、计费、权限、Provider、API、持久化、UI、文案/布局或配置 | 修改、聚焦回归、对应风险检查、真实入口证据 | 只有发现明确 Build 触发器才升级 |
| `build` | 完整新能力、结构性重构、破坏性演进、广泛回归或协调式发布 | case 驱动实现、`feature-plan.md`、`acceptance.json`、评审记录 | 本地完成后进入 `ship` |
...
| Skill | 典型场景 | 主要结果 | 下一步 |
| --- | --- | --- | --- |
| `ship` | 已验证的 Lite 或 Build 增量准备进入测试 | 测试环境部署、对应回归、人工验收、合并主干 | 人类选择是否 `deploy` |
...
| 一个局部热修必须当天发布 | `lite` → `ship` → `deploy` | 发布增加门禁和证据，不升级开发路线 |
| 沿用现有任务/下载链路新增一个导出动作 | `lite` | 边界清楚且复用既有架构 |
| 新建具有独立生命周期和存储的完整导出子系统 | `demo` → `prd` → `testcase` → `build` | 完整新能力命中 Build 触发器 |
| “做一个 AI 学习教练”，用户和价值还不清 | `brainstorm` → `demo` → `prd` → `testcase` → `build` | 依次解决意图、体验、真相和证明 |
| 一个局部结算交互还没定 | 直接澄清或 `demo` → 重新分类 | 产品不确定性需要定义，但不选择 Build |
| 局部权限判断 + 兼容性 API 字段 | `lite` | 权限/API 风险只追加 allow/deny 与兼容性检查 |
| 替换共享权限模型及公共 API | `demo` → `prd` → `testcase` → `architect` → `build` | 共享核心替换和公共边界变化命中 Build |
...
1. **从用户动作写起。** 请求里说清“谁从哪个真实入口做什么，看到什么结果”，不要只给
   “做完某模块”这种内部任务名。
2. **按工程深度和改动半径选路。** 边界清楚且沿用现有架构的工作默认 `lite`；只有明确命中
   完整新能力、共享核心重构、破坏性演进、广泛回归或协调式发布才走 Build。风险和发布只
   追加检查。纯文案、静态外观、文档默认不跑 E2E。
...
**独立 Builder** 仍然负责产品判断。意图清楚的边界改动直接走 `lite`，即使它改变行为或需要
发布；只有命中明确 Build 触发器时，才走完 demo/PRD/testcase、审阅报告，再进入 build。
...
- `deploy` 只负责生产：固定 release set、在最终主干刷新收据、备份/回滚、部署、prod-safe 回归、
  观察窗口；每个破坏性步骤都需要人类确认。
- `oncall` 先分诊和证据化诊断。回滚、热修复和缓解由人选择；热修按同一 Lite/Build 触发器
  分类，再进入 `ship` → `deploy`，加速不等于不验证。
...
- `skills/`：13 个用户入口；
- `shared/core-contract.md`：工程深度选路、证据、完成等级、反馈和安全底线；
